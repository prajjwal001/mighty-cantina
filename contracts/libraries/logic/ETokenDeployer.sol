...
library ETokenDeployer {
    function deploy(string memory name_, string memory symbol_, uint8 decimals_, address underlyingAsset_, uint256 id)
        external
        returns (address)
    {
        address eTokenAddress = address(
            new ExtraInterestBearingToken{
                salt: keccak256(abi.encode(underlyingAsset_, id, "MightyInterestBearingToken"))
