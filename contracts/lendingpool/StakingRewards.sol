...
contract StakingRewards is Ownable, IStakingRewards {
    ...
    function earned(address user, address rewardToken, uint256 curRewardPerToken) internal view returns (uint256) {
        uint256 d = curRewardPerToken - userRewardPerTokenPaid[user][rewardToken];
        ...
        updateReward(address(0))
    {
        ...
        rewardData[rewardToken].rewardRate = totalRewards / (endTime - startTime);
     ...
     *
     */
    function withdrawByLendingPool(uint256 amount, address user, address to)
        external
        onlyLendingPool
        nonReentrant
        updateReward(user)
