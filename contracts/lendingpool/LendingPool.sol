...
import "../PaymentsUpgradeable.sol";
...
contract LendingPool is ILendingPool, OwnableUpgradeable, PaymentsUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public nextDebtPositionId;
    ...
    function initialize(address _addressRegistry, address _WETH9) public initializer {
        ...
        _initializePayments(_WETH9);
        nextReserveId = 1;
        nextDebtPositionId = 1;
    }
    ...
    function initReserve(address asset) external onlyOwner notPaused {
        ...
        address eTokenAddress = ETokenDeployer.deploy(name, symbol, decimals, asset, id);
        ...
        returns (uint256 eTokenAmount)
    {
        ...
        // emit event
        emit Deposited(reserveId, _msgSender(), onBehalfOf, amount, eTokenAmount, referralCode);
    }
        ...
        returns (uint256)
    {
        ...
        if (eTokenAmount == type(uint256).max) {
            eTokenAmount = IExtraInterestBearingToken(reserve.eTokenAddress).balanceOf(_msgSender());
        }
        ...
        uint256 underlyingTokenAmount = _redeem(reserveId, eTokenAmount, to, receiveNativeETH);

        emit Redeemed(reserveId, _msgSender(), to, eTokenAmount, underlyingTokenAmount);
        ...
        returns (uint256)
    {
        ...
        IStakingRewards(stakingPool).withdrawByLendingPool(eTokenAmount, _msgSender(), address(this));
        ...
        returns (uint256)
    {
        ...
        if (reserve.underlyingTokenAddress == WETH9 && receiveNativeETH) {
            IExtraInterestBearingToken(reserve.eTokenAddress).burn(address(this), eTokenAmount, underlyingTokenAmount);
            unwrapWETH9(underlyingTokenAmount, to);
        } else {
            // burn eTokens and transfer the underlying tokens to receiver
            IExtraInterestBearingToken(reserve.eTokenAddress).burn(to, eTokenAmount, underlyingTokenAmount);
        }
    ...
    function borrow(address onBehalfOf, uint256 debtId, uint256 amount) external notPaused nonReentrant {
        ...
        require(amount <= reserve.availableLiquidity(), Errors.LP_NOT_ENOUGH_LIQUIDITY_TO_BORROW);
    ...
    function createStakingPoolForReserve(uint256 reserveId) internal {
        ...
        reserves[reserveId].stakingAddress = StakingRewardsDeployer.deploy(eTokenAddress);
    ...
    function setBorrowingRateConfig(
        ...
        reserve.borrowingRateConfig.utilizationA =
            uint128(Precision.FACTOR1E18.mul(utilizationA).div(Constants.PERCENT_100));

        reserve.borrowingRateConfig.borrowingRateA =
            uint128(Precision.FACTOR1E18.mul(borrowingRateA).div(Constants.PERCENT_100));
        reserve.borrowingRateConfig.utilizationB =
            uint128(Precision.FACTOR1E18.mul(utilizationB).div(Constants.PERCENT_100));
        reserve.borrowingRateConfig.borrowingRateB =
            uint128(Precision.FACTOR1E18.mul(borrowingRateB).div(Constants.PERCENT_100));
        reserve.borrowingRateConfig.maxBorrowingRate =
            uint128(Precision.FACTOR1E18.mul(maxBorrowingRate).div(Constants.PERCENT_100));
    }

    function getReserve(uint256 reserveId) internal view returns (DataTypes.ReserveData storage reserve) {
    ...
    function getTreasury() internal view returns (address treasury) {
        treasury = AddressRegistry(addressRegistry).getAddress(AddressId.ADDRESS_ID_TREASURY);
        require(treasury != address(0), Errors.VL_TREASURY_ADDRESS_NOT_SET);
    }

    function getVault(uint256 vaultId) internal view returns (address vaultAddress) {
        address vaultFactory = AddressRegistry(addressRegistry).getAddress(AddressId.ADDRESS_ID_VAULT_FACTORY);
    ...
    /**
     * @notice Get the status of the reserves
     * @param reserveIdArr The array of reserve ids
     * @return statusArr The array of reserve statuses
     */
        ...
        returns (ReserveStatus[] memory statusArr)
    {
        ...
        for (uint256 i = 0; i < reserveIdArr.length; i++) {
            ...
            statusArr[i].underlyingTokenAddress = reserves[reserveIdArr[i]].underlyingTokenAddress;
            statusArr[i].eTokenAddress = reserves[reserveIdArr[i]].eTokenAddress;
            statusArr[i].stakingAddress = reserves[reserveIdArr[i]].stakingAddress;
            (statusArr[i].totalLiquidity, statusArr[i].totalBorrows) =
                reserves[reserveIdArr[i]].totalLiquidityAndBorrows();
            statusArr[i].exchangeRate = reserves[reserveIdArr[i]].eTokenToReserveExchangeRate();
            statusArr[i].borrowingRate = reserves[reserveIdArr[i]].borrowingRate();
        }
    }
        ...
        returns (PositionStatus[] memory statusArr)
    {
        ...
        for (uint256 i = 0; i < reserveIdArr.length; i++) {
            ...
            statusArr[i].eTokenStaked = IStakingRewards(reserves[reserveIdArr[i]].stakingAddress).balanceOf(user);
            statusArr[i].eTokenUnStaked = IERC20(reserves[reserveIdArr[i]].eTokenAddress).balanceOf(user);
            statusArr[i].liquidity = statusArr[i].eTokenStaked.add(statusArr[i].eTokenUnStaked).mul(
                reserves[reserveIdArr[i]].eTokenToReserveExchangeRate()
    ...
    function getCurrentDebt(uint256 debtId) external view returns (uint256 currentDebt, uint256 latestBorrowingIndex) {
        ...
        currentDebt = debtPosition.borrowed.mul(latestBorrowingIndex).div(debtPosition.borrowedIndex);
    }
    ...
    function setBorrowingRateConfig(
        ...
        emit SetInterestRateConfig(
            reserveId, utilizationA, borrowingRateA, utilizationB, borrowingRateB, maxBorrowingRate
        );
    }
